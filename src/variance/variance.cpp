...
void Variance::setCurData(decltype(cur_data) data) {
  if (!this->cur_data.empty()) this->history += this->cur_data;
  this->cur_data = data;
  this->cur_mean = 0;
  this->cur_var = 0;
...
double Variance::getVariance() { return this->var; }

void Variance::present() {
  for (double v : this->cur_data) std::cout << v << " <- cur_data" << std::endl;
  for (double v : this->history) std::cout << v << " <- history" << std::endl;
